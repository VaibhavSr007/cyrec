        ...
        {images.map((imgDetails, imgIndex) => (
          <ReactCardFlip key={imgIndex} isFlipped={imgIndex === flipped} flipDirection='horizontal'>
            <div className='relative hover:cursor-pointer'>
              <img className='w-72 h-56 md:w-96 md:h-72 bg-contain bg-center rounded-3xl' src={imgDetails.img} />
            ...
            <div onClick={() => setFlipped(-1)} className='w-72 h-56 md:w-96 md:h-72 flex justify-center items-center bg-[#64CCC5] rounded-3xl hover:cursor-pointer'>{imgDetails.desc}</div>
          </ReactCardFlip>
        ))}
      </div>
